...
class CPU:
    ...
    def read(self, addr):
        ...
        if 0x0000 <= addr < 0x2000: return self.ram[addr % 0x0800]
            ...
            # Simplified NROM behavior:
            # For 16KB ROM (1 bank), 0x8000-0xBFFF maps to bank 0, 0xC000-0xFFFF mirrors bank 0.
            # For 32KB ROM (2 banks), 0x8000-0xBFFF maps to bank 0, 0xC000-0xFFFF maps to bank 1 (or last bank).
            # Current code effectively treats PRG ROM as contiguous from 0x8000.
            # A mapper would handle banking here.
            if self.nes.rom.prg_banks == 1: # 16KB ROM
                return self.nes.rom.prg_rom[(addr - 0x8000) % 0x4000] # Mirror 0x8000-0xBFFF to 0xC000-0xFFFF
            else: # 32KB ROM (or more, but NROM typically max 32KB)
                return self.nes.rom.prg_rom[addr - 0x8000] # Accesses full 32KB range if available
        return 0 
    ...
    def write(self, addr, value):
        ...
        elif 0x6000 <= addr < 0x8000: pass # SRAM/PRG-RAM
        elif 0x8000 <= addr <= 0xFFFF: pass # Mapper regs

    def read_word(self, addr): return self.read(addr) | (self.read(addr + 1) << 8)
    def read_word_bug(self, addr): # Page boundary bug for indirect JMP
        low = self.read(addr)
    ...
    def step(self):
        ...
        if not op_info:
            ...
            self.cycles_remaining = 1 # Minimal recovery
            return 1 
...
class PPU:
    ...
    def read_register(self, reg_idx):
        ...
        if reg_idx == 2: # PPUSTATUS
            ...
            if self.scanline == VBLANK_SCANLINE_START and self.cycle <= 3: # NMI suppression window
                 self.nmi_output = False 
        elif reg_idx == 4: val = self.oam[self.oamaddr] # OAMDATA
        elif reg_idx == 7: # PPUDATA
            val = self.data_buffer 
            self.data_buffer = self.read_ppu_memory(self.vram_addr_v) 
            if self.vram_addr_v >= 0x3F00: # Palette RAM reads are immediate
                 idx = self.vram_addr_v & 0x1F
                 if idx==0x10 or idx==0x14 or idx==0x18 or idx==0x1C: idx-=0x10
                 val = self.palette_ram[idx]
                 if self.ppumask & 0x01: val &= 0x30 # Greyscale
    ...
    def write_register(self, reg_idx, value):
        ...
        elif reg_idx == 1: self.ppumask = value # PPUMASK
        elif reg_idx == 3: self.oamaddr = value # OAMADDR
        elif reg_idx == 4: self.oam[self.oamaddr] = value; self.oamaddr=(self.oamaddr+1)&0xFF # OAMDATA
    ...
    def _get_physical_vram_index(self, ppu_addr): # For $2000-$2FFF
        m = self.nes.rom.mirroring # 0:V, 1:H
        if m == 1: return (ppu_addr&0x3FF) + (0x400 if ppu_addr&0x400 else 0) # H: $2000/$2800 -> NT A; $2400/$2C00 -> NT B
        else: return (ppu_addr&0x3FF) + (0x400 if ppu_addr&0x800 else 0)       # V: $2000/$2400 -> NT A; $2800/$2C00 -> NT B

    def read_ppu_memory(self, addr):
        addr &= 0x3FFF
        if addr >= 0x3000 and addr <= 0x3EFF: addr -= 0x1000 # Mirror $2xxx
    ...
    def step_ppu(self):
        if self.scanline == -1: # Pre-render
            if self.cycle == 1:
                self.ppustatus &= ~(StatusFlags.V | StatusFlags.B | 0x20) # V, S0, SO
                self.nmi_occurred_this_frame = False; self.nmi_output = False
        ...
        if -1 <= self.scanline < VISIBLE_SCANLINES: # Visible and Pre-render
            ...
            if self.is_rendering_enabled():
                if (2 <= self.cycle < 258) or (322 <= self.cycle < 338): # BG Fetch cycles
                    ...
                    elif fc_phase == 6: # Pattern Hi
                        pt_base = 0x1000 if (self.ppuctrl&0x10) else 0
                        fine_y = (self.vram_addr_v>>12)&7
                        self.bg_pattern_hi_latch = self.read_ppu_memory(pt_base + self.bg_nt_latch*16 + fine_y + 8)
                    elif fc_phase == 7: self._increment_scroll_x() # After all 4 bytes for a tile
        ...
        if self.scanline == VBLANK_SCANLINE_START and self.cycle == 1:
            if not self.nmi_occurred_this_frame:
                self.ppustatus |= StatusFlags.V # Set VBlank
                if self.ppuctrl & 0x80: self.nmi_output = True
                self.nmi_occurred_this_frame = True
        
        if 0 <= self.scanline < VISIBLE_SCANLINES and 1 <= self.cycle <= SCREEN_WIDTH: # Render pixel
            pixel_x = self.cycle - 1
            bg_pixel_color_idx = self.read_ppu_memory(0x3F00) # Default to universal BG
            bg_pixel_pattern_val = 0 # Low 2 bits of palette index (0 for transparent)

            if self.ppumask & 0x08: # BG enabled
            ...
            # Sprite Rendering (simplified for now, uses overlay)
            # A full scanline renderer would mux sprite pixel here.
            final_pixel_color_idx = bg_pixel_color_idx

            # Sprite Zero Hit (simplified check at pixel level)
            if self.sprite_zero_hit_possible and self.sprite_zero_being_rendered and \
               ...
               pixel_x != 255: # S0 hit cannot happen at x=255
                
                # Check if BG or Sprite rendering is disabled in the leftmost 8 pixels
                no_hit_due_to_clipping = False
                if pixel_x < 8:
                    if not (self.ppumask & 0x02): no_hit_due_to_clipping = True # BG hidden
                    if not (self.ppumask & 0x04): no_hit_due_to_clipping = True # Sprites hidden
                
                if not no_hit_due_to_clipping and bg_pixel_pattern_val != 0: # BG must be opaque
                    # Check if sprite 0 pixel is opaque at this x
                    # This requires sprite 0 shifters to be active and checked
                    # For simplicity, we assume sprite_zero_being_rendered implies it's active.
                    # A more complex check would involve sprite 0's shifter values.
                    # For now, if sprite 0 is on this scanline and BG is opaque, trigger (simplified).
                    self.ppustatus |= 0x40 # Set S0 hit
            
            # Draw pixel to framebuffer
            if not (self.ppumask & 0x02) and pixel_x < 8 : # Hide left 8px BG
        ...
        if self.cycle > PPU_CYCLES_PER_SCANLINE -1:
            ...
            if self.scanline > SCANLINES_PER_FRAME - 2:
                 ...
                 self._render_sprites_overlay() # Overlay sprites for simplicity
                 # pygame.display.flip() - REMOVED (Tkinter handles display update)
    ...
    def _evaluate_sprites_for_scanline(self, target_scanline):
        ...
        for i in range(len(self.secondary_oam)): self.secondary_oam[i] = 0xFF # Clear secondary OAM

        num_found = 0
        for i in range(0, 256, 4): # Iterate primary OAM
            y = self.oam[i] # OAM Y is actual top scanline of sprite
            if y >= 0xF0 : continue # Simple check to discard sprites way off screen

            # Sprite is on scanline if y <= target_scanline < y + height
        ...
        self.sprite_zero_being_rendered = False # Reset for current scanline processing
        for i in range(self.sprite_count_scanline):
            ...
            pt_base_sprites = (0x1000 if (self.ppuctrl & 0x08) else 0) if sprite_height == 8 \
                              else (0x1000 if (tile_id & 0x01) else 0) # 8x16, LSB of tile_id selects table
            
            current_tile = tile_id
            if sprite_height == 16:
                current_tile &= 0xFE # Use even tile for top
    ...
    def _render_sprites_overlay(self): # Simplified overlay render
        if not (self.ppumask & 0x10): return
        sprite_height = 16 if (self.ppuctrl & 0x20) else 8
        
        for i in range(252, -1, -4): # Iterate OAM backwards (sprite 63 to 0 for priority)
            y_coord = self.oam[i] # Actual Y of sprite top
            tile_id = self.oam[i+1]
            attr = self.oam[i+2]
            x_coord = self.oam[i+3]

            if y_coord >= 0xF0: continue # Skip if sprite Y is too far down
            ...
            pt_base_sprites = (0x1000 if (self.ppuctrl & 0x08) else 0) # For 8x8 or PPUCTRL bit 3
            ...
            for row in range(sprite_height):
                ...
                if sprite_height == 16:
                    final_pt_base = 0x1000 if (tile_id & 0x01) else 0 # LSB of OAM tile_id chooses table
                    current_tile &= 0xFE # Top tile is even
                    if flip_v: eff_row = (sprite_height - 1) - row # Flip 16px unit
                    if eff_row >= 8: current_tile += 1; eff_row -= 8 # Bottom half
                elif flip_v: eff_row = (sprite_height - 1) - row # 8x8 flip
                ...
                for bit_idx in range(8):
                    ...
                    if not (self.ppumask & 0x04) and draw_x < 8: continue # Sprite left 8px clip
                    ...
                    if palette_idx_low == 0: continue # Transparent

                    if priority_behind:
                        bg_color_tuple = self.framebuffer.get_at((draw_x, draw_y))[:3]
                        univ_bg_idx = self.read_ppu_memory(0x3F00) & 0x3F
                        if bg_color_tuple != NES_PALETTE[univ_bg_idx]: continue # BG in front
...
class APU:
    ...
    def play_frame_sync_sound(self):
        if self.sound_enabled and self.channel and self.test_sound and not self.channel.get_busy():
            # self.channel.play(self.test_sound) # Can be noisy
            pass
...
class Controller:
    ...
    def update_state(self, events, keys_pressed):
        ...
        for key,idx in self.key_map.items(): # Ensure held keys are registered if event missed
    ...
    def read(self):
        if self.strobe: self.index=0 # Always read button A if strobe is on
        if self.index >= 8: return 0x01 # Open bus after all buttons read
        val = 0x01 if self.buttons[self.index] else 0x00
        if not self.strobe: self.index += 1
        return val # Controller port typically returns D0 only, other bits can be open bus (0x40 typical)
...
class NES:
    ...
    def step_frame(self):
        ...
        target_cycles = 29781 # NTSC CPU cycles per frame
        
        # Ensure NMI flags are correctly reset for the new frame PPU processing.
        # self.ppu.nmi_output = False # PPU sets this when VBlank starts
        # self.ppu.nmi_occurred_this_frame = False # PPU manages this internally

        while self.total_cpu_cycles_this_frame < target_cycles:
            if self.ppu.nmi_output:
                self.ppu.nmi_output = False # CPU consumes NMI
                self.cpu.nmi()
...
class EmulatorGUI:
    def __init__(self, nes_instance):
        ...
        sf_h = (self.tk_win_h - 70) // SCREEN_HEIGHT # -70 for controls and padding
        self.scale = max(1, min(sf_w, sf_h))
        self.cnv_w, self.cnv_h = SCREEN_WIDTH*self.scale, SCREEN_HEIGHT*self.scale
        
        cf = tk.Frame(self.root); cf.pack(pady=10, expand=True) # Center canvas
        ...
        tk.Button(ctrl_f, text="Pause", command=self.toggle_pause, width=10).pack(side=tk.LEFT,padx=5)
        self.pause_btn = ctrl_f.winfo_children()[-1] # Get ref to button
    ...
    def reset_nes(self):
        ...
        self.nes.ppu.frame_count = 0 # Reset PPU frame count on manual reset
...
base_addr = 0xC000
# Offsets for the simple PRG code
# Original structure: init(12), wait_vblank1(6), main_loop(6), ctrl_read(6), beq(2), a_pressed_code(6 -> 3 for lda/sta + 3 for jmp), no_a_pressed_code(6 -> 3 for lda/sta + 3 for jmp)
# Total initial length: 12+6+6+6+2+6+6 = 44 bytes
# Offsets: init (0-11), wait_vblank1 (12-17), main_loop (18-23), ctrl_read (24-29), beq (30-31), 
# a_pressed_code_lda_sta (32-34), a_pressed_jmp (35-37)
# no_a_pressed_code_lda_sta (38-40), no_a_pressed_jmp (41-43)
simple_prg_code_orig = bytes([
    0x78,0xD8,0xA2,0xFF,0x9A,0xA9,0x90,0x8D,0x00,0x20,0xA9,0x1E,0x8D,0x01,0x20, # Init (ends at 14)
    0xAD,0x02,0x20,0x29,0x80,0xF0,0xFA, # Wait VBlank1 (15-21, BEQ to self)
    0xAD,0x02,0x20,0x29,0x80,0xF0,0xFA, # Main Loop (22-28, BEQ to self)
    0xAD,0x16,0x40,0x29,0x01, # Ctrl Read (29-33)
    0xF0,0x00, # BEQ _no_button_a_pressed (offset 34, placeholder target)
    0xA9,0x16,0x8D,0x00,0x3F, # A pressed: LDA #$16, STA $3F00 (offset 36-40)
    0x4C,0x00,0x00, # JMP _main_loop_end (offset 41, placeholder target)
    0xA9,0x12,0x8D,0x00,0x3F, # No A pressed: LDA #$12, STA $3F00 (offset 44, is _no_button_a_pressed, ends 48)
    0x4C,0x00,0x00  # JMP _main_loop (offset 49, is _main_loop_end, placeholder target)
])
simple_prg_list = list(simple_prg_code_orig)
# Patch BEQ at offset 34. PC=34+2=36. Target is offset 44. Rel jump = 44-36 = 8 (0x08)
simple_prg_list[34+1] = 0x08
# Patch JMP at offset 41. Target is offset 49. Addr = C000 + 49
jmp_target1 = base_addr + 49
simple_prg_list[41+1] = jmp_target1 & 0xFF; simple_prg_list[41+2] = jmp_target1 >> 8
# Patch JMP at offset 49. Target is offset 22 (start of main_loop LDA $2002). Addr = C000 + 22
...
nmi_handler_code = bytes([0xA9,0x28,0x8D,0x01,0x3F,0x40]) # LDA #$28, STA $3F01, RTI
...
prg[0x3FFC:0x3FFE] = (base_addr&0xFF, base_addr>>8) # Reset Vec -> C000
prg[0x3FFA:0x3FFC] = ((base_addr+0x100)&0xFF, (base_addr+0x100)>>8) # NMI Vec -> C100
prg[0x3FFE:0x4000] = (base_addr&0xFF, base_addr>>8) # IRQ Vec -> C000 (BRK uses this)
SAMPLE_ROM_DATA.extend(prg)
chr_data = bytearray([0]*8192); 
chr_data[16:24]=b'\xAA'*8; chr_data[24:32]=b'\x55'*8 # Tile 1
chr_data[32:48]=b'\xFF'*16 # Tile 2
...
def main_tkinter():
    ...
    try:
        # To load from file:
        # rom_path = "your_rom.nes"
        # if os.path.exists(rom_path): rom_to_load = open(rom_path, "rb").read()
        # else: print(f"{rom_path} not found, using sample."); rom_to_load = SAMPLE_ROM_DATA
        # nes = NES(rom_to_load)
        nes = NES(SAMPLE_ROM_DATA)
    except Exception as e: print(f"ROM/NES init error: {e}"); pygame.quit(); return
    gui = EmulatorGUI(nes); gui.start() # Blocks until GUI closed
    pygame.quit(); print("Emulator closed. Bye-bye, meow!")

if __name__ == "__main__":
    main_tkinter()import asyncio
import platform
import pygame
import numpy as np
import tkinter as tk
